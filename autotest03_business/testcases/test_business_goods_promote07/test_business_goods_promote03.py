...
1.正常流程
2.判定结果并且回滚测试数据，因为每一次的正确流程都会把1改为0，所以需要改回来，数据库数据尽量不要修改
3.追加其他case
"""
...
class TestGoodsPromote(unittest.TestCase):
    ...
    def test_goods_promote(self, case):
        ...
        if case.case_id == 15:
            res = self.db.find_one('select * from tb_goods where isPromote = 0 and isOnSale = 0 and goodsId != %s'
                                   'order by rand() limit 1', (get_ini_data('goods1', 'goodsId'),))
        if case.case_id == 16:
            res = self.db.find_one('select * from tb_goods where isPromote = 1 and isOnSale = 1 and goodsId != %s'
                                   'order by rand() limit 1', (get_ini_data('goods1', 'goodsId'),))
        ...
        try:
            ...
            if case.case_id in (1, 16):
                # 验证数据库，断言响应结果已经更改完成
                count = self.db.find_count('select * from tb_goods where goodsId = %s and isPromote = 0', (goodsId,))
                self.assertEqual(1, count)
                # 数据回滚，将数据还原
                self.db.cud('update tb_goods set isPromote = 1 where goodsId = %s', (goodsId,))
