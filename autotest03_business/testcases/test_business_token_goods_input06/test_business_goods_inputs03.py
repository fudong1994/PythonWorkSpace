...
class TestGoodsInput(unittest.TestCase):
    ...
    def test_goods_input(self, case):
        ...
        if case.case_id in (1, 7, 13, 14, 17, 23):
            self.db.cud('delete from tb_goods where goodsName = %s', (eval(case.case_data)["goodsName"],))
        if case.case_id == 6:
            self.db.cud('delete from tb_goods where goodsName = %s', (eval(case.case_data)["goodsName"],))
            self.db.cud('insert into tb_goods(goodsName,goodsTypeId,num,shopPrice) values(%s,%s,%s,%s)',
                        (eval(case.case_data)["goodsName"], eval(case.case_data)["goodsTypeId"],
                         eval(case.case_data)["number"], eval(case.case_data)["shopPrice"]))
